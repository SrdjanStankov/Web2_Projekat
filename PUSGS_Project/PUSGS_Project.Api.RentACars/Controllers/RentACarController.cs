...
namespace PUSGS_Project.Api.RentACars.Controllers
{
    ...
    public class RentACarController : ApiController
    {
        ...
        // GET: api/<controller>
        [HttpGet]
        public async Task<IEnumerable<RentACarModel>> GetAsync() => new HashSet<RentACarModel>((await repository.GetAllAsync()).Select(s => new RentACarModel(s)));

        // GET api/<controller>/5
        [HttpGet("{id}")]
        public async Task<RentACarModel> GetAsync(int id) => new RentACarModel(await repository.GetAsync(id));
